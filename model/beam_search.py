...
class SequenceBeamSearch(object):
    ...
    def search(self, initial_ids, initial_cache):
        ...
        :param initial_ids: [batch_size] zeros
        :param initial_cache: dict
        :return:
        """
    ...
    def _create_initial_state(self, initial_ids, initial_cache):
        ...
        """
            initial_log_probs
                tf.Tensor([[  0. -inf -inf -inf]], shape=(1, 4), dtype=float32)
            alive_log_probs
                tf.Tensor(
                        [[  0. -inf -inf -inf]
                         [  0. -inf -inf -inf]
                         [  0. -inf -inf -inf]
                         [  0. -inf -inf -inf]], shape=(4, 4), dtype=float32)
        """
        initial_log_probs = tf.constant([[0.] + [-float("inf")] * (self.beam_size - 1)])

        alive_log_probs = tf.tile(initial_log_probs, [self.batch_size, 1])

        # Expand all values stored in the dictionary tile to the beam size,
        # so that each beam has a separate cache.
        ...
        #   [batch_size, beam_size, 1] shape of zeros
        finished_seq = tf.zeros(tf.shape(alive_seq), tf.int32)

        # Set scores of the initial finished seqs to negative infinity.
        #   [batch_size, beam_size] shape of -INFs
        finished_scores = tf.ones([self.batch_size, self.beam_size]) * -INF

        # Initialize finished flags with all False values.
        #   [batch_size, beam_size] shape of bool
    ...
    def _grow_alive_seq(self, state):
        ...
        """
            log_probs = tf.constant([[2, 3, 4, 6, 7, 8, 1], [3, 10, 3, 5, 7, 9, 8]])
            topk_log_probs, topk_indices = tf.nn.top_k(log_probs, k=4)
            
            topk_log_probs = tf.Tensor(
                    [[ 8  7  6  4]
                     [10  9  8  7]], shape=(2, 4), dtype=int32)
            topk_indices = tf.Tensor(
                    [[5 4 3 2]
                     [1 5 6 4]], shape=(2, 4), dtype=int32)
        """
